...
# Create a mini copy of original database
create table if not exists title_basics_mini (tconst char(10) not null, titleType varchar(100), primaryTitle varchar(500), originalTitle varchar(500), isAdult boolean, startYear char(4), endYear char(4), runtimeMinutes int, genres varchar(100), primary key (tconst));
create table if not exists title_crew_mini (tconst char(10) not null, directors text, writers text, primary key (tconst));
create table if not exists title_principals_mini (tconst char(10) not null, ordering int not null, nconst char(10), category varchar(100), job varchar(500), characters varchar(500), primary key (tconst, ordering));
create table if not exists title_ratings_mini (tconst char(10) not null, averageRating numeric(3,1), numVotes int, primary key(tconst));
create table if not exists name_basics_mini (nconst char(10) not null, primaryName varchar(200), birthYear char(4), deathYear char(4), primaryProfession varchar(100), knownForTitles varchar(100), primary key(nconst));